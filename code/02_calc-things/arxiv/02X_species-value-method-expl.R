...
#---this is old I think, don't run 9 oct 2025
...
#--look at the weeds
#--this is a good figure, make it a supplemental?
d7 %>%
  ggplot(aes(eppo_code, sc_value2_max)) +
...
# 8. relationship btwn the categories -------------------------------------

library(ggcorrplot)

d7 %>% 
  pivot_wider(names_from = cat, values_from = sc_value2_max) %>% 
  mutate(harm = abs(harm)) %>% 
  ggplot() +
  geom_point(aes(ecocont, harm), color = "red") +
  geom_point(aes(ecocont, pollinator), color = "blue")
...
d8 <- 
  d7 %>% 
    ...
    mutate(harm = abs(harm)) %>% 
    ungroup() %>% 
    select(ecocont, harm, pollinator)
