...
res <- 
  as_tibble(emmeans(m2_t2, ~ cctrt_id*till_id*yearF, type = 'response'))
  

# 3. model on perennial weeds------------------------------------------------------
...
hist(d_perenn$count2)

#--full model, doesn't converge
m3_t1 <- glmmTMB(count2 ~ yearF * till_id * straw_id * cctrt_id
                 + (1 | block_id/straw_id/till_id/cctrt_id),
                 family = tweedie(),
                 data = d_perenn)
