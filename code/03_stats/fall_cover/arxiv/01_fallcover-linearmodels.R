...
(emmeans(m1, specs = pairwise ~ cctrt_id:weayear))$contrasts |> 
  ...
  arrange(cctrt1, p.value) |> 
  mutate(psimp = round(p.value, 3)) |> 
  write_xlsx("data/stats/emmeans/emmeans-pairs-fallcover-soil-cctrt-btwn-years.xlsx")



# model on soil cover-------------------------------------------------------------------

#--run same model but on soil fraction
m2 <- glmmTMB(cover_frac ~ till_id * cctrt_id * straw_id * weayear +
                (1|block_id/straw_id/till_id/cctrt_id),
              family=ordbeta(link = "logit"), 
              data=d_cat |> filter(cover_cat == "soil"))
...
#--write final model anova table
tidy(car::Anova(m2)) |> 
  write_xlsx("data/stats/anova/anova_fallcover-soil.xlsx")
...
#--straw and cctrt interactions with weayear, are they crossover or amplifying?
