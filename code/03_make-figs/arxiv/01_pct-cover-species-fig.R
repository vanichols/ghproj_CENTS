...
y %>% 
  ...
  filter(cover_cat != "soil",
         cover_cat != "covercrop") %>% 
  ggplot(aes(x = block_id, eppo_code)) +
  geom_point(aes(size = cover_pct, color = eppo_code)) +
  facet_nested(straw_id + till_id ~ year + cctrt_id) +
...
y %>% 
  ...
  mutate(year = year(date2)) %>% 
  #filter(year == 2018) %>% 
  left_join(y %>% mutate(year = year(date2)) %>% expand(eppo_code, year)) %>% 
  filter(cover_cat != "soil",
         cover_cat != "covercrop") %>% 
  mutate(cover_pct = ifelse(cover_pct == 0, NA, cover_pct)) %>% 
  ggplot(aes(x = block_id, eppo_code)) +
  geom_tile(aes(fill = cover_pct)) +
  facet_nested(straw_id + till_id ~ year + cctrt_id) +
