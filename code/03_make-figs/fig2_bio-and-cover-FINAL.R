...
cov_stats <- 
  read_csv("data/stats/figs_emmeans/emmeans-fig2-fallcover.csv") |>  
  mutate(year = parse_number(weayear),
         labelval_mod1 = round(response * 100, 0),
         cover_cat2 = "Cover crop") |> 
  select(till_id, cctrt_id, year, labelval_mod1, cover_cat2) %>% 
  MakeNiceLabels2(.)

#--need to replace labelval1 with modelled values
a6 <- 
  a5 %>%
  left_join(cov_stats) |> 
  mutate(
    cover_cat2 = fct_inorder(cover_cat2),
    cover_cat2rev = fct_rev(cover_cat2)) %>% 
  ...
  mutate(cumpct = cumsum(cover_pct),
         ...
         labelval_mod = ifelse(!is.na(labelval_mod1), paste0(labelval_mod1, "%"), NA)) 
...
p2 <-
  ...
  geom_col(aes(x = till_nice, 
               y = cover_pct/100, 
               fill = cover_cat2rev), color = "black") +
...
ggsave("figs/fig2_bio-and-cover-FINAL.png", 
       width = 10, height = 7)
