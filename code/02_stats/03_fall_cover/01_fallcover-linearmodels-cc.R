...
#--the model fits well, but the zeros become a problem in nocc surface
#--the comparisons with nocc will be nonsense (never sig.)
...
#--yeah, it is just going to look funny
plot(m2_simres)
...
# write model estimates for figure ----------------------------------------

emmeans(m0, specs = ~ till_id|cctrt_id|weayear, type = "response") |>
  as_tibble() |> 
  write_csv("data/stats/figs_emmeans/emmeans-fig2-fallcover.csv")
...
#--patterns are bascially the same across years except for MixE
emmeans(m0, specs = ~ till_id|cctrt_id|weayear, type = "response") |> 
  as_tibble() |> 
  ggplot(aes(reorder(cctrt_id, response), response, color = cctrt_id)) +
  geom_point() +
...
#--tillage? cctrt ranking stayed the same more or less
...
(emmeans(m0, specs = pairwise ~ cctrt_id:weayear))$contrasts |> 
  ...
  write_xlsx("data/stats/supp_tables/fallcover-cc-ccwea-pairs.xlsx")

#--within a cctrt
#--sig change in mixes
#--radL was solid
#--nocc and radM were slightly variable
