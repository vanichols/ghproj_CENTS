#--use emmeans estimates for yield figure
#--9 april updated to include yields separated by tillage effect (which was sig)
...
w <- cents_wea
y <- read_csv("data/stats/figs_emmeans/emmeans-fig1-yields.csv")
yraw <- cents_cropyields
#cents_cropyields
...
#p4 <- 
  ...
  geom_linerange(aes(ymin = lower.CL,
                     ...
                     color = crop), 
                 show.legend = F) +
  geom_text( data = . %>% filter(cctrt_id == "rad_M"), 
             aes(x = "RadM", y = estimate + 0.75, label = "*"), size = 10) +
...
p4 <- 
  ...
  geom_point(aes(shape = crop), 
             ...
             size = 2) +
  geom_text( data = . %>% filter(cctrt_id == "rad_M"), 
             aes(x = "RadM", y = estimate + 1, label = "*"), size = 10) +
  scale_y_continuous(limits = c(0, 6)) +
  scale_shape_manual(values = c(15, 17, 19)) +
  scale_color_manual(values = c(ylw1, dkbl1, c2)) +
  scale_fill_manual(values = c(ylw1, dkbl1, c2)) +
