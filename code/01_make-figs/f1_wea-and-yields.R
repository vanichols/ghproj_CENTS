...
p4 <- 
  ...
  geom_jitter(aes(color = crop, shape = crop), 
              width = 0.1, alpha = 1, show.legend = F, size = 4) +
...
(p3 + p2 + p1) / p4 + plot_layout(guides = 'collect') & theme(legend.position = 'top')

ggsave("figs/fig1_wea-and-yields.png", width = 9, height = 6)
